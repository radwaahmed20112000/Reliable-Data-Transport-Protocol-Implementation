#include <stdio.h>
#include <stdlib.h>
#include <sys/types.h>
#include <iostream>
#include <fstream>
#include <string>
#include <cstdio>
#include <cstdlib>
#include <unistd.h>
#include <cstring>
#include <netinet/in.h>
#include <sys/socket.h>
#include <arpa/inet.h>
#include <arpa/inet.h>
#include "chrono"


#define MAXLINE 1024

using namespace std;

void write_file(char *data){
    int rcv_bytes = strlen(data);
    string fileName = "rcv.txt";

    FILE *file_pointer = fopen(fileName.c_str(), "w");
    if(file_pointer == NULL)
    {
        printf("File Cannot be opened.\n");
        exit(EXIT_FAILURE);
    }

    printf("%s\n",data);
    if((fwrite(data, sizeof(char), rcv_bytes, file_pointer)) < rcv_bytes)
    {
        perror("File write failed! ");
        exit(EXIT_FAILURE);
    }

    printf("File recieved successfully!\n");
    fclose(file_pointer);
}

// Driver code
int main() {

    struct ack_packet buffer[MAXLINE];
    int n;
    socklen_t len;
    uint32_t waiting_seqno = 0;
    uint32_t last_recieved_ack;
    struct packet response;
    int client_socket;
    struct sockaddr_in server_addr;
    bool file_ack = false;
    bool accepted = true;

    //reading from the file

    ifstream requestFile("client.in");
    string temp;
    string fileName;
    getline (requestFile, temp);
    const char* IP = temp.c_str();

    getline (requestFile, temp);
    in_port_t port_number = atoi(temp.c_str());

    getline (requestFile, temp);
    fileName = temp;

    requestFile.close();


    // Creating socket file descriptor
    client_socket = socket(AF_INET, SOCK_DGRAM, 0);
    if(client_socket < 0) {
        perror("Error in socket");
        exit(1);
    }
    printf("Client socket created successfully.\n");


    // Filling server information
    server_addr.sin_family = AF_INET;
    int rtnVal = inet_pton(AF_INET, IP, &server_addr.sin_addr);
    if (rtnVal == 0) {
        perror("inet_pton() failed, invalid address string\n");
        exit(1);
    } else if (rtnVal < 0) {
        perror("inet_pton() failed\n");
        exit(1);
    }
    server_addr.sin_port = htons(port_number);

    char file[fileName.size()+1];
    strcpy(file, fileName.c_str());
    struct packet file_datagram;
    request_packet_init(&file_datagram,file,0);

    // sends the file name and waits for an ack
    while(!file_ack){

        sendto(client_socket, (struct packet *) &file_datagram , sizeof(struct packet*),MSG_CONFIRM, (const struct sockaddr *) &server_addr,sizeof(server_addr));
        cout << "Hello message sent.\n" ;

        time_t start = time(0);
        double diff ;
        while( difftime(time(0) , start)< 6 && !n){
            n = recvfrom(client_socket, (char *)buffer, MAXLINE,MSG_WAITALL, (struct sockaddr *) &server_addr,&len);
        }

        if(n){
            file_ack = true;
        }

        cout << "Server : %s\n" << buffer << endl;
    }




        while(1){

            n = recvfrom(client_socket, (struct packet *) &response, sizeof(struct packet*),MSG_WAITALL, (struct sockaddr *) &server_addr,&len);
            cout << "client : %d\n" << response.seqno<< endl;

            //check the end of the file
            if(response.data == NULL){
                break;
            }

            // recieving the wanted packet
            if(response.seqno == waiting_seqno){
                struct ack_packet new_ack;
                ack_packet_init(&new_ack,response.seqno);
                write_file(response.data);

                sendto(client_socket, (struct ack_packet *) &new_ack, sizeof(struct ack_packet*),MSG_CONFIRM, (const struct sockaddr *) &server_addr,sizeof(server_addr));
                cout << "client : %d\n" << new_ack.ackno << endl;


                waiting_seqno++;
                last_recieved_ack = new_ack.ackno;
            }

            // recieving not in order packet
            else{

                struct ack_packet new_ack;
                ack_packet_init(&new_ack,last_recieved_ack);

                sendto(client_socket, (struct ack_packet *) &new_ack, sizeof(struct ack_packet*),MSG_CONFIRM, (const struct sockaddr *) &server_addr,sizeof(server_addr));
                cout << "client : %d\n" << new_ack.ackno << endl;

            }


        }


    close(client_socket);
    return 0;
}